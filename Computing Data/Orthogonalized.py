...
def create_ortho(column):
    x1 = np.array(macro_data.iloc[2:, 0]).reshape((-1, 1))
    x2 = np.array(macro_data.iloc[2:, 1]).reshape((-1, 1))
    x3 = np.array(macro_data.iloc[2:, 2]).reshape((-1, 1))
    x4 = np.array(macro_data.iloc[2:, 3]).reshape((-1, 1))
    x5 = np.array(macro_data.iloc[2:, 4]).reshape((-1, 1))
    x = np.hstack((x1, x2, x3, x4, x5))
    y = np.array(column)
    pro = LinearRegression().fit(x, y)
    coeff = pro.coef_
    alpha = pro.intercept_
    df = pd.DataFrame(index=macro_data.iloc[2:, :].index)
    df[macro_data.columns[0]] = x1
    df[macro_data.columns[1]] = x2
    df[macro_data.columns[2]] = x3
    df[macro_data.columns[3]] = x4
    df[macro_data.columns[4]] = x5
    df['Actual'] = y
...
def orthogonalizing(data):
    ...
    for i in range(len(cols)):
        df[cols[i]] = create_ortho(data.iloc[0:-1, i])
    return df
...
def get_pca_split_new(rawdata, list1, list2):
    ...
    ortho.extend(list2)
    orthodata = orthogonalizing(rawdata[ortho].iloc[1:, :])
...
# Attention with option 1
    ...
    attention, v1_at, v2_at, co_at, corrs_at, pca_a = get_ratio(
...
# Sentiment
    ...
    sentiment, v1_se, v2_se, co_sent, corrs_se, pca_s = get_ratio(
        data_sent, list2)
...
def combined_data(r, freq, list1, list2):
    ...
    ratio_att, ratio_sent, vars, cors, corrs, pca = get_pca_split_new(
        rawdata, list1, list2)
...
for i in range(6):
    ...
    print(ticker)
    df, cor, corr, pcas = combined_data(
        i, frequency, attention_list, sentiment_list)
...
for i in range(len(finaldata)):
    filepath = directory+"/SavedData/Orthogonalized/"+tickers[i]+"Ratios.xlsx"
    save_xls(finaldata[i], finaltickers[i], filepath)

for i in range(len(finaldata)):
    filepath = directory+"/SavedData/Orthogonalized/"+tickers[i]+"Raw.xlsx"
