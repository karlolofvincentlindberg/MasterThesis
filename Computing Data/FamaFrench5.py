...
# Fama-French Data
filepath = directory+"/Data/FamaFrench5.xlsx"
ff_data_d = pd.read_excel(filepath, index_col=0,
                          engine=None, sheet_name="Daily")
ff_data_m = pd.read_excel(filepath, index_col=0,
                          engine=None, sheet_name="Monthly")
...
def create_ortho(column, freq):
    ...
    x4 = np.array(df_o.iloc[:, 3]).reshape((-1, 1))
    x5 = np.array(df_o.iloc[:, 4]).reshape((-1, 1))
...
for i in range(len(finaldata)):
    filepath = directory+"/SavedData/Frenched/"+tickers[i]+"5Ratios.xlsx"
    save_xls(finaldata[i], finaltickers[i], filepath)

for i in range(len(finaldata)):
    filepath = directory+"/SavedData/Frenched/"+tickers[i]+"5Raw.xlsx"
