...
for j in range(len(frequencies)):
    ...
    for i in range(len(tickers)):
        ...
        df_em3_1.iloc[2*i+2, 2*j+1] = f"({round(t_s1[2],2)})"

# Gathering all exponential models in one dataframe
dataframes = [df_em1_1, df_em2_1, df_em3_1]
datalabels = ["em1_1_ff", "em1_2_ff", "em1_3_ff"]

for i in range(len(dataframes)):
    hint = datalabels[i]
    with open(directory+'/LatexTables/Regressions/Exponential/' + hint+'5new.tex', 'w') as tf:
