    ...
    for j in range(len(frequencies)):
        ...
        for i in range(len(tickers)):
            ...
            if p_value<0.05:
                ...
                dfmat_log.iloc[r,c] = "Yes"
dfmat_log.to_excel(f"{directory}/Stationarity/newLogResultsStationarity.xlsx")

#Stationarity of computed ratios for linear regressions
#Daily
df_d_log = dfmat_log.transpose().iloc[:,:6]
df_d_log.columns = tickers
with open(directory+'/LatexTables/Stationarities/Stationarity_d_lognew.tex','w') as tf:
    tf.write(df_d_log.to_latex())

#Weekly
df_w_log = dfmat_log.transpose().iloc[:,6:12]
df_w_log.columns = tickers
with open(directory+'/LatexTables/Stationarities/Stationarity_w_lognew.tex','w') as tf:
    tf.write(df_w_log.to_latex())

#Monthly
df_m_log = dfmat_log.transpose().iloc[:,12:]
df_m_log.columns = tickers
with open(directory+'/LatexTables/Stationarities/Stationarity_m_lognew.tex','w') as tf:
