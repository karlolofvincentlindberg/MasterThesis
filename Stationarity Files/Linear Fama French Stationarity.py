    ...
    for j in range(len(frequencies)):
        ...
        for i in range(len(tickers)):
            ...
            if p_value < 0.05:
                ...
                dfmat_ff.iloc[r, c] = "Yes"
dfmat_ff.to_excel(f"{directory}/Stationarity/FFResultsStationarity.xlsx")
...
with open(directory+'/LatexTables/Stationarities/Stationarity_d_ff.tex', 'w') as tf:
    tf.write(df_d.to_latex())

# Weekly
df_w = dfmat_ff.transpose().iloc[:, 6:12]
df_w.columns = tickers
df_w = df_w.drop(['Log Return - ADF', 'Log Return - KPSS'])
with open(directory+'/LatexTables/Stationarities/Stationarity_w_ff.tex', 'w') as tf:
    tf.write(df_w.to_latex())

# Monthly
df_m = dfmat_ff.transpose().iloc[:, 12:]
df_m.columns = tickers
df_m = df_m.drop(['Log Return - ADF', 'Log Return - KPSS'])
with open(directory+'/LatexTables/Stationarities/Stationarity_m_ff.tex', 'w') as tf:
