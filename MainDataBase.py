...
# company names
stocks = ["amazon stock", "apple stock", "facebook stock",
          "google stock", "microsoft stock", "netflix stock"]
tickers = ["AMZN", "AAPL", "META", "GOOGL", "MSFT", "NFLX"]
new_columns = ['Date', 'Price', 'TV', 'TC', 'TPC', 'TNC', 'NC', 'NPC',
               'NNC', 'SI', 'SIR', 'VMP', 'PCR', 'SVT - T', 'SVI - S', 'SVI', 'SHOUT']
...
def read_data(r, dataframe, mindt, maxdt, freq):
    name = dataframe[r]
    stock = stocks[r]
    ticker = tickers[r]
...
# Inserting the put call - ratio
    ...
    for i in range(len(data)):
        ...
        datta = datta[datta['Ticker Symbol'] == ticker]
        calls = datta[datta['Type'] == 'C']
        puts = datta[datta['Type'] == 'P']
        x = puts['Volume'].sum()/calls['Volume'].sum()
        data.iloc[i, -1] = x
...
# Including the shares outstanding
    ...
    for j in range(len(data)):
        da = shout[shout['Ticker Symbol'] == ticker]
...
for i in range(len(companies_m)):
    data = read_data(i, companies_m, mindate, maxdate, "m")
    raw_m.append(data)

# Weekly data
for i in range(len(companies_w)):
    data = read_data(i, companies_w, mindate, maxdate, "w")
    raw_w.append(data)

# Daily data
for i in range(len(companies_d)):
    data = read_data(i, companies_d, mindate, maxdate, "d")
    raw_d.append(data)
